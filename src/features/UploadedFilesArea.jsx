...
import { FiExternalLink, FiSearch, FiX, FiFile, FiCalendar } from 'react-icons/fi';
...
const UploadedFilesArea = () => {
    ...
    const failedFiles = useMemo(() =>
        files.filter(f => ['failed', 'duplicate'].includes(f.status)),
        [files]
    );

    const hasErrors = failedFiles.length > 0;
    const years = useMemo(() =>
        [...new Set(successfulFiles.map(f => f.year))].sort((a, b) => b - a),
        [successfulFiles]
    );

    // Фильтрация для модального окна
    const filteredFiles = useMemo(() => {
        let result = successfulFiles.filter(file =>
    ...
    const handleResolveError = (filename) => {
        const confirmed = window.confirm(
            'Убедитесь, что вы устранили ошибку, прежде чем её удалять'
        );
                    ...
                    {successfulFiles.slice(0, 10).map((file) => (
                        <li key={file.filename} className={styles.fileItem}>
                            ...
                            <span className={styles.fileMeta}>
                                ...
                                {file.year} • {file.status}
                            </span>
                        </li>
                    ))}

                    {successfulFiles.length === 0 && (
                        <li className={styles.empty}>Нет загруженных файлов</li>
                    )}
                </ul>
            </div>
                ...
                {hasErrors ? (
                    <>
                        <p className={styles.warningText}>
                            Не забудьте загрузить файлы, в которых были ошибки.
                            Иначе ваш отчет может быть неточным.
                        </p>
                                ...
                                >
                                    <div className={styles.errorFileContent}>
                                        <div className={styles.errorFileHeader}>
                                            ...
                                            <span className={styles.errorFileMeta}>
                                                {file.year} • {file.status} •{' '}
                ...
                <div className={styles.modalContent}>
                    ...
                    <div className={styles.modalInfo}>
                        <p className={styles.fileCount}>
                            Найдено: <strong>{filteredFiles.length}</strong> файлов
                        </p>
                    </div>
